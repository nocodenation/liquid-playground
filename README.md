...
Located in `python_extensions/GetMicrosoftMail`, this processor fetches emails from a Microsoft 365 / Outlook account using the Graph API.

- **Features**:
  - Fetches emails using OAuth 2.0 (Authorization Code Flow).
  - Supports filtering with standard OData queries (e.g., `isRead eq false`).
  - Can output full JSON metadata or raw MIME email content.
  - Optionally marks messages as read.
- **Setup**:
  - Requires a `token.json` file containing the Refresh Token and Client Config. This is generated by the `MicrosoftOAuthManager` processor.
  - The location of this file is configurable in the processor properties (default recommendation: `/files/microsoft-token.json`).
...
Located in `python_extensions/MicrosoftOAuthManager`, this processor handles the OAuth 2.0 flow for obtaining Microsoft Graph credentials.

- **Features**:
  - Generates Authorization URLs for Microsoft Login.
  - Exchanges Authorization Codes for Access/Refresh tokens.
  - Returns a JSON object containing both tokens and client configuration, ready for use by `GetMicrosoftMail`.
- **Setup**:
  - Create an App Registration in Microsoft Entra ID (Azure AD).
  - Add a **Web** platform Redirect URI (e.g., `http://localhost:8999/callback` for local dev).
  - Configure the processor with Client ID, Client Secret, and Tenant ID.
- **Flow Construction**:
  1.  **HandleHttpRequest** (Listens on `/login` and `/callback`).
  2.  **MicrosoftOAuthManager**:
      - If input path is `/login` -> Generates URL -> Redirect user (302/307).
      - If input path is `/callback` (with `code` param) -> Exchanges code -> Returns JSON Token.
  3.  **PutFile**: Save the JSON Token to a file (e.g., `microsoft-token.json`).
  4.  **HandleHttpResponse**: Return success page (HTML).
